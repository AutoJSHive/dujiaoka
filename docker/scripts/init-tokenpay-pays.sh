...
# TokenPay 支付方式初始化脚本
# 在独角数卡数据库中添加 TokenPay 支付方式

DB_NAME="${DB_DATABASE:-dujiaoka}"
TOKENPAY_DIR="/opt/tokenpay"
LOCK_FILE="${TOKENPAY_DIR}/.pays_initialized"
...
# 读取 API Token
if [ -f "${TOKENPAY_DIR}/.api_token" ]; then
    API_TOKEN=$(cat "${TOKENPAY_DIR}/.api_token")
else
...
if [ "$EXISTING" != "0" ]; then
    echo ">>> TokenPay payment methods already exist, updating API token..."
...
echo ">>> Note: Payment methods are disabled by default. Enable them in admin panel after configuring wallet addresses."
